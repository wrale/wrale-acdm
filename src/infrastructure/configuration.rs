...
use crate::domain::{Dependency, Configuration, RepositoryType, DomainError};
...
impl TomlConfigurationRepository {
    ...
    fn domain_to_toml(&self, config: &Configuration) -> ConfigFile {
        let sources = config.dependencies.iter().map(|dep| {
        ...
        ConfigFile {
            location: config.default_location.as_ref().map(|l| l.to_string_lossy().to_string()),
            sources,
        }
    }
    
    fn toml_to_domain(&self, config_file: ConfigFile) -> Result<Configuration, DomainError> {
        let dependencies = config_file.sources.into_iter().map(|source| {
            let repository_type = match source.repo_type.as_str() {
                "git" => RepositoryType::Git,
                unsupported => return Err(DomainError::ConfigurationError(
                    format!("Unsupported repository type: {}", unsupported)
                )),
            };
...
impl ConfigurationRepository for TomlConfigurationRepository {
    fn load(&self, path: &Path) -> Result<Configuration, DomainError> {
        let mut file = File::open(path).map_err(|e| DomainError::ConfigurationError(
            format!("Failed to open configuration file: {}", e)
        ))?;
        
        let mut contents = String::new();
        file.read_to_string(&mut contents).map_err(|e| DomainError::ConfigurationError(
            format!("Failed to read configuration file: {}", e)
        ))?;
        
        let config_file: ConfigFile = toml::from_str(&contents).map_err(|e| DomainError::ConfigurationError(
    ...
    fn save(&self, config: &Configuration, path: &Path) -> Result<(), DomainError> {
        ...
        let toml_string = toml::to_string_pretty(&config_file).map_err(|e| DomainError::ConfigurationError(
        ...
        let mut file = File::create(path).map_err(|e| DomainError::ConfigurationError(
            format!("Failed to create configuration file: {}", e)
        ))?;
        
        file.write_all(toml_string.as_bytes()).map_err(|e| DomainError::ConfigurationError(
