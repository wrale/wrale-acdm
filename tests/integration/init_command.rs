...
"#;
    ...
    let assert = cmd.arg("--config").arg(&config_path).arg("init").assert();
    ...
    assert_ne!(
        content_after_force, existing_content,
        "Config file should have been overwritten with --force"
    );
    assert!(
        content_after_force.contains("sources = []"),
        "New config should contain empty sources"
    );
