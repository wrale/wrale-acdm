...
use wrale_acdm::domain::repositories::{RepositoryFetcher, FileSystemManager};
...
fn test_dependency_update_with_mocks() {
    ...
    repo_fetcher
        .expect_fetch()
        .with(
            eq("git@github.com:example/repo.git"), 
            eq("main"), 
            always()
        )
        .times(1)
    ...
    assert!(result.is_ok(), "Expected update to succeed, got: {:?}", result);
}
...
"#).unwrap();
    ...
    assert!(output.status.success(), "Command failed with: {:?}", std::str::from_utf8(&output.stderr).unwrap_or("Unknown error"));
    
    // Read the updated config file
    let config_content = fs::read_to_string(&config_path).unwrap();
    
    // Assert that the paths were added
    assert!(config_content.contains("docs/**"), "docs/** path not found in config");
    assert!(config_content.contains("src/**"), "src/** path not found in config");
}
