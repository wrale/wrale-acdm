...
fn test_git_operations_status() {
    ...
    let is_repo = git_ops
        .is_git_repository(repo_dir.path())
        .expect("Failed to check repository status");
    ...
    fs::write(repo_dir.path().join("file2.txt"), "Unstaged content")
        .expect("Failed to create unstaged file");
    ...
    assert!(
        ...
        "Expected status to show untracked files"
    );
    assert!(
        !status.is_clean(),
        "Expected unclean repository status with untracked files"
    );
    ...
    assert!(
        !status.is_clean(),
        "Expected unclean repository status with staged changes"
    );

    // Modify a tracked file without staging
    fs::write(repo_dir.path().join("file1.txt"), "Modified content")
    ...
    assert!(
        ...
        "Expected status to show unstaged changes"
    );
    assert!(
        !status.is_clean(),
        "Expected unclean repository status with unstaged changes"
    );
}
